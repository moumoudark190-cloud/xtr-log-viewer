...
impl SearchState {
    ...
    fn next(&mut self) -> Option<usize> {
        ...
        if self.first_search {
            ...
            if self.current_match_idx == 0 { if self.wrap_around { self.current_match_idx = self.matches.len() - 1; } }
...
fn light_visuals() -> egui::Visuals {
    ...
    v.panel_fill          = Color32::from_rgb(242, 246, 255);
    v.window_fill         = Color32::from_rgb(255, 255, 255);
    v.override_text_color = Some(Color32::from_rgb(28, 32, 42));
...
fn accent_button_ui(text: &str) -> Button<'_> {
    Button::new(RichText::new(text).strong().color(BG_BASE).font(FontId::proportional(12.0)))
        .fill(COL_ACCENT).stroke(Stroke::NONE).rounding(Rounding::same(7.0)).min_size(Vec2::new(0.0, 30.0))
}
...
fn icon_button(icon: &str) -> Button<'_> {
    Button::new(RichText::new(icon).color(COL_MUTED).font(FontId::proportional(14.0)))
        .fill(Color32::from_rgb(24, 30, 40)).stroke(Stroke::new(0.5, COL_BORDER))
        .rounding(Rounding::same(7.0)).min_size(Vec2::new(30.0, 30.0)).sense(Sense::click())
}

fn close_button() -> Button<'static> {
    Button::new(RichText::new("✕").color(Color32::from_rgb(160, 160, 165)).font(FontId::proportional(13.0)))
        .fill(Color32::TRANSPARENT).stroke(Stroke::NONE).rounding(Rounding::same(4.0)).min_size(Vec2::new(28.0, 28.0))
}
...
fn render_match_context(painter: &egui::Painter, pos: egui::Pos2, mat: &SearchMatch, max_width: f32) {
    ...
    let before = if mat.context_before.len() > 20 { format!("…{}", &mat.context_before[mat.context_before.len()-20..]) } else { mat.context_before.clone() };
    let after  = if mat.context_after.len()  > 30 { format!("{}…", &mat.context_after[..30])  } else { mat.context_after.clone()  };
    let match_text = if mat.match_text.len() > 50 { format!("{}…", &mat.match_text[..49]) } else { mat.match_text.clone() };
    ...
    let mut x = pos.x;
    if !before.is_empty() { painter.text(egui::pos2(x, pos.y), Align2::LEFT_CENTER, &before, font.clone(), COL_MUTED); x += before_width; }
...
fn menu_item(icon: &str, label: &str, shortcut: &str) -> Button<'static> {
    let text = if shortcut.is_empty() { format!("{}  {}", icon, label) } else { format!("{}  {}   {}", icon, label, shortcut) };
    Button::new(RichText::new(text).font(FontId::proportional(12.0)).color(COL_TEXT))
        .fill(Color32::TRANSPARENT).stroke(Stroke::NONE).min_size(Vec2::new(190.0, 26.0))
}
...
impl LogViewerApp {
    ...
    fn export_filtered(&mut self) {
        if let Some(path) = rfd::FileDialog::new().add_filter("Log", &["log","txt"]).set_file_name("filtered.log").save_file() {
            let content = self.filtered.iter().filter_map(|&i| self.all_lines.get(i)).map(|l| l.raw.as_str()).collect::<Vec<_>>().join("\n");
    ...
    fn export_search_results(&mut self) {
        if let Some(path) = rfd::FileDialog::new().add_filter("Text", &["txt"]).add_filter("CSV", &["csv"]).set_file_name("search_results.txt").save_file() {
            ...
            let content = if is_csv {
                ...
                let mut lines = vec![format!("Search: \"{}\"  ({} matches)", self.search.find_what, self.search.matches.len()), "─".repeat(80), String::new()];
                for m in &self.search.matches {
                    let mod_s = if m.module.len() > 12 { &m.module[..12] } else { &m.module };
                    lines.push(format!("Line {:>5} │ {:>4} │ {:>12} │ {}[{}]{}", m.line_num, m.level.label(), mod_s, m.context_before, m.match_text, m.context_after));
                }
                lines.join("\n")
            };
            match std::fs::write(&path, content) {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut Frame) {
        ...
        egui::TopBottomPanel::top("menubar")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.menu_button(RichText::new("File").font(FontId::proportional(12.0)).color(COL_MUTED), |ui| {
                        ui.set_min_width(210.0);
                        ui.spacing_mut().item_spacing.y = 2.0;
                        if ui.add(menu_item("📂", "Open…",             "Ctrl+O")).clicked() { self.open_file_dialog(); ui.close_menu(); }
                        ...
                        ui.add_enabled_ui(!self.all_lines.is_empty(), |ui| {
                            if ui.add(menu_item("💾", "Export Filtered…",        "")).clicked() { self.export_filtered(); ui.close_menu(); }
                            if ui.add(menu_item("📋", "Export Search Results…",  "")).clicked() { self.export_search_results(); ui.close_menu(); }
                        });
                        ui.add(egui::Separator::default().spacing(4.0));
                        if ui.add(menu_item("🗑", "Clear",  "")).clicked() { self.clear_file(); ui.close_menu(); }
                        ui.add(egui::Separator::default().spacing(4.0));
                        if ui.add(menu_item("✕", "Exit",   "Alt+F4")).clicked() { ctx.send_viewport_cmd(egui::ViewportCommand::Close); }
                    });

                    ui.menu_button(RichText::new("Help").font(FontId::proportional(12.0)).color(COL_MUTED), |ui| {
                        ...
                        for (k, v) in [
                            ...
                            ui.horizontal(|ui| {
                                ui.add_sized([95.0, 20.0], egui::Label::new(RichText::new(k).font(FontId::monospace(10.0)).color(COL_ACCENT)));
        ...
        egui::TopBottomPanel::top("toolbar")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    // ── Hero search bar ───────────────────────────────────────
                    let sw = 340.0; let sh = 34.0;
                    let filter_active = !self.filter_text.is_empty();
                    let (srect, _) = ui.allocate_exact_size(Vec2::new(sw, sh), Sense::hover());
                    let painter = ui.painter();
                    let fill   = Color32::from_rgb(10, 13, 20);
                    let border = if filter_active { COL_ACCENT } else { COL_BORDER_HL };
                    painter.rect(srect, Rounding::same(10.0), fill, Stroke::new(1.5, border));
                    painter.text(egui::pos2(srect.min.x + 14.0, srect.center().y), Align2::LEFT_CENTER, "⌕",
                        FontId::proportional(17.0), if filter_active { COL_ACCENT } else { COL_FAINT });

                    let tr = egui::Rect::from_min_max(
                        egui::pos2(srect.min.x + 36.0, srect.min.y + 4.0),
                        egui::pos2(srect.max.x - if filter_active { 30.0 } else { 10.0 }, srect.max.y - 4.0));
                    let te = ui.put(tr, TextEdit::singleline(&mut self.filter_text)
                        .hint_text(RichText::new("Filter log…").color(COL_FAINT))
                        .frame(false).font(FontId::monospace(13.0)));
                    if te.changed() { self.apply_filters(); }

                    if filter_active {
                        let xr = egui::Rect::from_center_size(egui::pos2(srect.max.x - 16.0, srect.center().y), Vec2::splat(20.0));
                        let xresp = ui.interact(xr, egui::Id::new("filter_x"), Sense::click());
                        ui.painter().text(xr.center(), Align2::CENTER_CENTER, "✕", FontId::proportional(12.0),
                            if xresp.hovered() { COL_TEXT } else { COL_MUTED });
                        if xresp.clicked() { self.filter_text.clear(); self.apply_filters(); }
                    }

                    // ── Module combo ──────────────────────────────────────────
                    if !self.modules.is_empty() {
                        let lbl = if self.module_filter.is_empty() { "All modules".to_string() }
                            else if self.module_filter.len() > 18 { format!("…{}", &self.module_filter[self.module_filter.len()-16..]) }
                            else { self.module_filter.clone() };
                        let mut changed = false;
                        egui::ComboBox::from_id_source("mod_cb")
                            .selected_text(RichText::new(lbl).font(FontId::proportional(11.5)).color(COL_TEXT))
                            .width(150.0)
                            .show_ui(ui, |ui| {
                                if ui.selectable_label(self.module_filter.is_empty(),
                                    RichText::new("All modules").color(COL_MUTED).font(FontId::proportional(11.5))).clicked() {
                                    self.module_filter.clear(); changed = true;
                                }
                                for m in self.modules.clone() {
                                    let d = if m.len() > 26 { format!("…{}", &m[m.len()-24..]) } else { m.clone() };
                                    if ui.selectable_label(self.module_filter == m, RichText::new(d).font(FontId::proportional(11.5))).clicked() {
                    ...
                    for (idx, lbl, color) in defs {
                        if level_toggle(ui, lbl, self.counts[idx], self.show[idx], color) { self.show[idx] = !self.show[idx]; fc = true; }
                    }
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ...
                        if ui.add(icon_button(theme_icon)).on_hover_text("Toggle light/dark mode").clicked() {
                        ...
                        if ui.add(icon_button("A+")).on_hover_text("Larger font").clicked() {
                            self.font_size = (self.font_size+1.0).min(20.0); self.row_height = self.font_size+8.0;
                        }
                        if ui.add(icon_button("A-")).on_hover_text("Smaller font").clicked() {
                            self.font_size = (self.font_size-1.0).max(9.0); self.row_height = self.font_size+8.0;
                        }

                        ui.add(egui::Separator::default().vertical().spacing(6.0));

                        let nav_active = self.nav_open;
                        let nav_lbl = if !self.nav_entries.is_empty() { format!("Nav  {}", self.nav_entries.len()) } else { "Nav".into() };
                        if ui.add(Button::new(RichText::new(nav_lbl).color(if nav_active { COL_ACCENT } else { COL_MUTED }).font(FontId::proportional(11.5)))
                            .fill(if nav_active { Color32::from_rgba_unmultiplied(88,166,255,20) } else { Color32::from_rgb(24,30,40) })
                            .stroke(Stroke::new(if nav_active {1.0} else {0.5}, if nav_active { Color32::from_rgba_unmultiplied(88,166,255,140) } else { COL_BORDER }))
                            .rounding(Rounding::same(7.0)).min_size(Vec2::new(0.0, 30.0)))
                            .on_hover_text("Navigation panel  Ctrl+N").clicked()
                        { self.nav_open = !self.nav_open; }

                        let wrap_lbl = if self.wrap_lines { "↩ Wrap" } else { "→ Wrap" };
                        if ui.add(Button::new(RichText::new(wrap_lbl)
                            .color(if self.wrap_lines { COL_ACCENT } else { COL_MUTED }).font(FontId::proportional(11.5)))
        ...
        egui::TopBottomPanel::bottom("statusbar")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ui.label(RichText::new(format!("{} / {} lines", self.filtered.len(), self.all_lines.len()))
                    ...
                    .show(ctx, |ui| {
                        ui.horizontal(|ui| {
                            ...
                            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                ...
                                if ui.add(Button::new(RichText::new(if is_bm { "♥ Bookmarked" } else { "♡ Bookmark" })
                                    .color(if is_bm { Color32::from_rgb(255,140,200) } else { COL_MUTED })
                ...
                .show(ctx, |ui| {
                    ...
                    if total_h > 0.0 && viewport_h > 0.0 {
                        ...
                        painter.rect(egui::Rect::from_min_max(egui::pos2(r.min.x+0.5,wy0),egui::pos2(r.max.x-0.5,wy1)),
                            Rounding::same(2.0), Color32::from_rgba_unmultiplied(200,225,255,16),
        ...
        if self.nav_open && !self.all_lines.is_empty() {
            ...
            egui::SidePanel::right("nav_panel")
                ...
                .frame(egui::Frame::none().fill(Color32::from_rgb(13,16,24)).stroke(Stroke::new(1.0,COL_BORDER)))
                .show(ctx, |ui| {
                    ui.spacing_mut().item_spacing = Vec2::new(6.0,4.0);
                    egui::Frame::none().fill(Color32::from_rgb(16,20,30)).inner_margin(egui::Margin::symmetric(10.0,8.0)).show(ui, |ui| {
                        ui.horizontal(|ui| {
                    ...
                    egui::Frame::none().fill(Color32::from_rgb(12,15,22)).inner_margin(egui::Margin::symmetric(10.0,8.0)).show(ui, |ui| {
                        ...
                        egui::Grid::new("nav_flt").num_columns(2).spacing([8.0,4.0]).show(ui, |ui| {
                            macro_rules! cb { ($f:expr, $l:expr, $k:expr) => {{ let c=$k.color(); ui.checkbox(&mut $f, RichText::new($l).color(c).font(FontId::monospace(10.5))); }}; }
                            cb!(self.nav_show_error,     "ERR",      NavKind::Error);     cb!(self.nav_show_warning,   "WRN",         NavKind::Warning);   ui.end_row();
                            cb!(self.nav_show_teststart, "Test ▶",   NavKind::TestStart); cb!(self.nav_show_testend,   "Test ■",      NavKind::TestEnd);   ui.end_row();
                            cb!(self.nav_show_step,      "Step",     NavKind::Step);      cb!(self.nav_show_teardown,  "Teardown",    NavKind::Teardown);  ui.end_row();
                            cb!(self.nav_show_custom,    "★ Custom", NavKind::Custom);    cb!(self.nav_show_bookmark,  "♥ Bookmarks", NavKind::Bookmark);  ui.end_row();
                        });
                        ui.add_space(5.0);
                        ui.horizontal(|ui| {
                            ui.label(RichText::new("Keyword:").font(FontId::monospace(9.5)).color(COL_FAINT));
                            let kr = ui.add(egui::TextEdit::singleline(&mut new_kw).hint_text("any text").desired_width(f32::INFINITY).font(FontId::monospace(10.5)));
                    ...
                    if visible_data.is_empty() {
                        ...
                        ui.vertical_centered(|ui| {
                            ui.label(RichText::new(if self.nav_entries.is_empty() { "No landmark lines\ndetected." } else { "All types filtered out." })
                        ...
                        ScrollArea::vertical().id_source("nav_scroll").auto_shrink(false).show(ui, |ui| {
                            ...
                            for (kind, row_idx, line_num, label) in &visible_data {
                                ...
                                let ir = egui::Frame::none()
                                    .fill(if is_sel { Color32::from_rgba_unmultiplied(88,166,255,24) } else { Color32::TRANSPARENT })
                                    ...
                                    .show(ui, |ui| {
                                        ui.horizontal(|ui| {
                                            ...
                                            ui.vertical(|ui| {
                                                ui.spacing_mut().item_spacing.y = 2.0;
                                                ui.horizontal(|ui| { ui.spacing_mut().item_spacing.x = 5.0; nav_kind_pill(ui, *kind); ui.label(RichText::new(format!("line {}", line_num)).font(FontId::monospace(9.0)).color(COL_FAINT)); });
                                ...
                                if interact.hovered() && !is_sel { ui.painter().rect_filled(ir.rect, Rounding::ZERO, Color32::from_rgba_unmultiplied(255,255,255,6)); }
            ...
            if let Some(row) = jump { self.scroll_to_offset = Some(row as f32*self.row_height); self.selected = Some(row); self.detail_open = true; }
        }
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                if self.all_lines.is_empty() {
                    ui.centered_and_justified(|ui| {
                        ui.vertical_centered(|ui| {
                            ...
                            ui.add_space(8.0);
                            ui.label(RichText::new("Better readability · Test output · Log exploration").size(13.0).color(COL_MUTED));
                            ui.add_space(28.0);
                            if ui.add(accent_button_ui("  📂  Open File  ")).clicked() { self.open_file_dialog(); }
                            ui.add_space(18.0);
                            ui.label(RichText::new("Ctrl+O  open  ·  Ctrl+F  find  ·  Ctrl+N  nav  ·  Esc  clear").size(11.5).color(COL_FAINT));
                        });
                    });
                    return;
                }
                ...
                // Column headers
                {
                    ...
                    let fid = FontId::monospace(9.5); let col = Color32::from_rgb(110,118,134);
                ...
                let out = sa.show_rows(ui, row_h, n, |ui, row_range| {
                    ...
                    for row_idx in row_range {
                        ...
                        let nav_kind: Option<NavKind> = self.nav_entries.iter().find(|e| e.row_idx==row_idx && e.kind!=NavKind::Bookmark).map(|e| e.kind);
                        ...
                        if is_bookmarked {
                            ui.painter().rect_filled(egui::Rect::from_min_size(row_rect.min, Vec2::new(3.0,row_h)), Rounding::ZERO, Color32::from_rgb(255,140,200));
                        }
                        if matches!(line.level, Level::Error|Level::Warning) {
                            let xo = if is_bookmarked { 3.0 } else { 0.0 };
                            ui.painter().rect_filled(egui::Rect::from_min_size(egui::pos2(row_rect.min.x+xo, row_rect.min.y), Vec2::new(2.5,row_h)), Rounding::ZERO, line.level.color());
                        }
                        if let Some(kind) = nav_kind {
                            let c = kind.color();
                            ui.painter().rect_filled(egui::Rect::from_min_size(egui::pos2(row_rect.max.x-3.0, row_rect.min.y), Vec2::new(3.0,row_h)), Rounding::ZERO, Color32::from_rgba_unmultiplied(c.r(),c.g(),c.b(),130));
                        }
                        ...
                        let msg_col = match line.level { Level::Error=>Color32::from_rgb(255,175,165), Level::Warning=>Color32::from_rgb(255,218,148), _=>Color32::from_rgb(205,210,222) };
...
impl LogViewerApp {
    fn render_find_dialog(&mut self, ctx: &egui::Context) {
        ...
        let dw = 490.0; let dh = 300.0;
        let pos = egui::pos2((screen.width()-dw)/2.0, 90.0);
        let mut close_req = false;

        Window::new("find_dlg_w")
            .id(egui::Id::new("find_dlg"))
            .fixed_pos(pos).fixed_size([dw, dh])
            ...
            .show(ctx, |ui| {
                ...
                painter.text(egui::pos2(rect.min.x+18.0, rect.center().y), Align2::LEFT_CENTER, "⌕", FontId::proportional(20.0), COL_ACCENT);
                painter.text(egui::pos2(rect.min.x+44.0, rect.center().y), Align2::LEFT_CENTER, "Find", FontId::proportional(14.0), COL_TEXT);

                if !self.search.matches.is_empty() {
                    let badge = format!("{} / {}", self.search.current_match_idx+1, self.search.matches.len());
                    let br = egui::Rect::from_center_size(egui::pos2(rect.max.x-62.0, rect.center().y), Vec2::new(80.0,24.0));
                    painter.rect_filled(br, Rounding::same(12.0), Color32::from_rgba_unmultiplied(88,166,255,40));
                    painter.text(br.center(), Align2::CENTER_CENTER, badge, FontId::monospace(11.0), COL_ACCENT);
                }
                ...
                let xcol = if cresp.hovered() { painter.rect_filled(cr, Rounding::same(5.0), Color32::from_rgb(50,20,20)); Color32::from_rgb(255,110,110) } else { COL_MUTED };
                painter.text(cr.center(), Align2::CENTER_CENTER, "✕", FontId::proportional(13.0), xcol);
                painter.line_segment([egui::pos2(rect.min.x,rect.max.y), egui::pos2(rect.max.x,rect.max.y)], Stroke::new(1.0,COL_BORDER));
                if cresp.clicked() { close_req = true; }

                // Body
                egui::Frame::none()
                    .inner_margin(egui::Margin{left:22.0,right:22.0,top:18.0,bottom:18.0})
                    .show(ui, |ui| {
                        ui.spacing_mut().item_spacing.y = 0.0;

                        // Input
                        let avail_w = ui.available_width();
                        let (irect, _) = ui.allocate_exact_size(Vec2::new(avail_w, 38.0), Sense::hover());
                        let painter = ui.painter();
                        let is_empty = self.search.find_what.is_empty();
                        painter.rect(irect, Rounding::same(8.0), Color32::from_rgb(9,12,18), Stroke::new(1.5, if !is_empty { COL_ACCENT } else { COL_BORDER_HL }));
                        let te_rect = egui::Rect::from_min_max(egui::pos2(irect.min.x+12.0,irect.min.y+4.0), egui::pos2(irect.max.x-12.0,irect.max.y-4.0));
                        let te_resp = ui.put(te_rect, TextEdit::singleline(&mut self.search.find_what)
                            .hint_text(RichText::new("Search for…").color(COL_FAINT))
                            .frame(false).font(FontId::monospace(13.0)));
                        if te_resp.changed() { self.search.first_search = true; self.search.find_all(&self.filtered, &self.all_lines); }
                        let enter_pressed = te_resp.lost_focus() && ui.input(|i| i.key_pressed(Key::Enter));

                        ui.add_space(16.0);

                        // Checkboxes (native egui, clean and compact)
                        ui.horizontal(|ui| {
                            ui.spacing_mut().item_spacing.x = 20.0;
                            let mut chg = false;
                            chg |= ui.checkbox(&mut self.search.match_case,  "Match case").changed();
                            chg |= ui.checkbox(&mut self.search.whole_word,  "Whole word").changed();
                            chg |= ui.checkbox(&mut self.search.wrap_around, "Wrap around").changed();
                            chg |= ui.checkbox(&mut self.search.backward,    "Backward").changed();
                            if chg { self.search.first_search = true; self.search.find_all(&self.filtered, &self.all_lines); }
                        });
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.add_space(4.0);
                            for (mode, lbl) in [(SearchMode::Normal,"Normal"),(SearchMode::Extended,"Extended (\\n \\t)"),(SearchMode::Regex,"Regex")] {
                                let sel = self.search.mode == mode;
                                if ui.add(Button::new(RichText::new(lbl).font(FontId::proportional(11.0)).color(if sel { COL_ACCENT } else { COL_MUTED }))
                                    .fill(if sel { Color32::from_rgba_unmultiplied(88,166,255,22) } else { Color32::TRANSPARENT })
                                    .stroke(if sel { Stroke::new(1.0, Color32::from_rgba_unmultiplied(88,166,255,100)) } else { Stroke::NONE })
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if ui.add(Button::new(RichText::new("▶  Find Next").strong().color(BG_BASE).font(FontId::proportional(12.0)))
                                .fill(COL_ACCENT).stroke(Stroke::NONE).rounding(Rounding::same(7.0)).min_size(Vec2::new(115.0,32.0))).clicked() || enter_pressed {
                                self.do_find_next();
                            }
                            if ui.add_enabled(has_m, Button::new(RichText::new("◀  Previous").color(COL_TEXT).font(FontId::proportional(12.0)))
                                .fill(Color32::from_rgb(28,34,46)).stroke(Stroke::new(1.0,COL_BORDER_HL))
                                .rounding(Rounding::same(7.0)).min_size(Vec2::new(95.0,32.0))).clicked() {
                                self.do_find_prev();
                            }
                            if ui.add(Button::new(RichText::new("☰  Find All").color(COL_ACCENT).font(FontId::proportional(12.0)))
                                .fill(Color32::from_rgba_unmultiplied(88,166,255,18)).stroke(Stroke::new(1.0,Color32::from_rgba_unmultiplied(88,166,255,80)))
                                .rounding(Rounding::same(7.0)).min_size(Vec2::new(85.0,32.0))).clicked() {
                                self.do_find_all_with_results();
                            }
                            ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                                if ui.add(Button::new(RichText::new("Close").color(COL_MUTED).font(FontId::proportional(11.5)))
                                    .fill(Color32::TRANSPARENT).stroke(Stroke::new(0.5,COL_BORDER))
                                    .rounding(Rounding::same(7.0)).min_size(Vec2::new(60.0,32.0))).clicked() { close_req = true; }
                            });
                        });
    ...
    fn render_results_panel(&mut self, ctx: &egui::Context) {
        ...
        egui::TopBottomPanel::bottom("results_panel")
            ...
            .frame(egui::Frame::none().fill(Color32::from_rgb(10,13,20)).stroke(Stroke::new(1.0,COL_BORDER)))
            .show(ctx, |ui| {
                self.search.results_panel_height = ui.available_height();

                egui::Frame::none().fill(Color32::from_rgb(14,18,26)).inner_margin(egui::Margin::symmetric(16.0,9.0)).show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("⌕").size(14.0).color(COL_ACCENT));
                        ui.add_space(6.0);
                        ui.label(RichText::new("Search Results").font(FontId::proportional(12.5)).color(COL_TEXT).strong());
                        ui.add_space(8.0);
                        egui::Frame::none().fill(Color32::from_rgba_unmultiplied(88,166,255,35)).rounding(Rounding::same(10.0)).inner_margin(egui::Margin::symmetric(10.0,3.0)).show(ui, |ui| {
                            ui.label(RichText::new(format!("{} matches", self.search.matches.len())).font(FontId::monospace(10.5)).color(COL_ACCENT));
                        });
                        ui.add_space(8.0);
                        ui.label(RichText::new(format!("for \"{}\"", self.search.find_what)).font(FontId::proportional(11.5)).color(COL_MUTED));
                        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                            ui.spacing_mut().item_spacing.x = 6.0;
                            if ui.add(Button::new(RichText::new("✕").color(COL_MUTED)).fill(Color32::TRANSPARENT).stroke(Stroke::NONE).min_size(Vec2::splat(26.0))).clicked() { close_panel = true; }
                            if ui.add(ghost_button("📋 Copy")).clicked() {
                                let t = self.search.matches.iter().map(|m| format!("Line {}: {}", m.line_num, m.match_text)).collect::<Vec<_>>().join("\n");
                ...
                egui::Frame::none().fill(Color32::from_rgb(13,16,22)).inner_margin(egui::Margin::symmetric(16.0,5.0)).show(ui, |ui| {
                    ui.horizontal(|ui| {
                        for (w, lbl) in [(55.0,"LINE"),(46.0,"LVL"),(80.0,"MODULE"),(200.0,"MATCH CONTEXT")] {
                            ui.add_sized([w,16.0], egui::Label::new(RichText::new(lbl).font(FontId::monospace(9.0)).color(COL_FAINT)));
                        }
                    });
                });
                ui.add(egui::Separator::default().spacing(0.0));
                ...
                ScrollArea::vertical().id_source("results_scroll").auto_shrink(false).show(ui, |ui| {
                    ...
                    for (idx, mat) in self.search.matches.iter().enumerate() {
                        ...
                        let painter = ui.painter();
                        let bg = if is_current { Color32::from_rgba_unmultiplied(88,166,255,32) } else if resp.hovered() { Color32::from_rgb(18,22,32) } else if idx%2==1 { Color32::from_rgb(13,16,22) } else { Color32::TRANSPARENT };
                        if bg != Color32::TRANSPARENT { painter.rect_filled(rect, Rounding::ZERO, bg); }
                        if is_current { painter.rect_filled(egui::Rect::from_min_size(rect.min, Vec2::new(3.0,rh)), Rounding::ZERO, COL_ACCENT); }
                        let lc = mat.level.color();
                        painter.rect_filled(egui::Rect::from_min_size(egui::pos2(rect.min.x+4.0,rect.min.y+5.0), Vec2::new(2.0,rh-10.0)), Rounding::same(1.0), lc);
                        let y = rect.center().y; let mut x = rect.min.x + 14.0;
                        painter.text(egui::pos2(x+42.0,y), Align2::RIGHT_CENTER, mat.line_num.to_string(), FontId::monospace(10.5), COL_MUTED); x += 60.0;
                        painter.text(egui::pos2(x,y), Align2::LEFT_CENTER, mat.level.label(), FontId::monospace(10.0), lc); x += 46.0;
                        let md = if mat.module.len()>10 { format!("{}…",&mat.module[..9]) } else { mat.module.clone() };
                        painter.text(egui::pos2(x,y), Align2::LEFT_CENTER, &md, FontId::monospace(10.0), Color32::from_rgb(130,140,160)); x += 80.0;
