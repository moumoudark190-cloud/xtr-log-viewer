...
impl LogViewerApp {
    ...
    fn recompute_find_matches(&mut self) {
        ...
        for (idx, &line_idx) in self.filtered.iter().enumerate() {
            ...
            let hay = if self.find_match_case { raw.clone() } else { raw.to_lowercase() };

            let found = match self.find_search_mode {
                SearchMode::Normal => {
                    if self.find_whole_word { self.matches_whole_word(&hay, &needle) }
                    else { hay.contains(&needle) }
                }
                SearchMode::Extended => {
                    let expanded_needle = self.expand_escapes(&needle);
                    let expanded_hay   = self.expand_escapes(&hay);
                    if self.find_whole_word { self.matches_whole_word(&expanded_hay, &expanded_needle) }
                    else { expanded_hay.contains(&expanded_needle) }
                }
                // Regex mode: treated as plain Normal search (regex crate not linked)
                SearchMode::Regex => {
                    if self.find_whole_word { self.matches_whole_word(&hay, &needle) }
                    else { hay.contains(&needle) }
                }
            ...
            if found { self.find_matches.push(idx); }
        }
    ...
    fn find_prev(&mut self) {
        if self.find_total_matches == 0 { return; }
        if self.find_current_match == 0 {
            if self.find_wrap_around { self.find_current_match = self.find_total_matches.saturating_sub(1); }
        } else {
            self.find_current_match -= 1;
        }
        self.find_next_called = true;
    ...
    fn add_to_find_history(&mut self) {
        if !self.find_what.is_empty() && !self.find_history.contains(&self.find_what) {
            ...
            if self.find_history.len() > 20 { self.find_history.pop(); }
        }
    }
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut Frame) {
        ...
        ctx.input(|i| {
            if i.key_pressed(Key::Escape) {
                if self.find_dialog_open { self.close_find_dialog(); }
            ...
            if i.key_pressed(Key::F3) {
                if i.modifiers.shift { self.find_prev(); } else { self.find_next(); }
            }
        });
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.menu_button("Search", |ui| {
                        ...
                        ui.separator();
                        if ui.button("Find Next  F3").clicked() { self.find_next(); ui.close_menu(); }
                        if ui.button("Find Previous  Shift+F3").clicked() { self.find_prev(); ui.close_menu(); }
                    });
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ...
                        if self.all_lines.is_empty() {
                            if ui.add(accent_button("📂 Open File")).clicked() { self.open_file_dialog(); }
                        } else {
                            if ui.add(primary_button("📂 Open")).clicked() { self.open_file_dialog(); }
                        }

                        if !self.all_lines.is_empty() {
                            if ui.add(primary_button("🗑 Clear")).clicked() { self.clear_file(); }
                            ui.add(egui::Separator::default().vertical().spacing(8.0));
        ...
        if self.find_dialog_open {
            ...
            let dialog_pos = egui::pos2((screen_rect.width() - 520.0) / 2.0, 120.0);
            ...
            Window::new("Find")
                ...
                .collapsible(false)
                .resizable(false)
                .frame(egui::Frame::window(&ctx.style()).fill(BG_PANEL).stroke(Stroke::new(1.0, COL_BORDER)))
                .show(ctx, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        for (tab, label) in [
                            ...
                            let (fg, bg) = if active { (COL_TEXT, Color32::from_rgb(45,55,70)) }
                                else { (COL_MUTED, Color32::TRANSPARENT) };
                    ...
                    ui.horizontal(|ui| {
                        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                            if ui.add(primary_button("Close")).clicked() { self.close_find_dialog(); }
                        });
                    });
                });
        }
            ...
            .show(ctx, |ui| {
                ...
                let out = sa.show_rows(ui, row_h, n, |ui, row_range| {
                    ...
                    for row_idx in row_range {
                        ...
                        let is_current_find  = is_find_match && self.find_matches.get(self.find_current_match) == Some(&row_idx);
                        ...
                        // Level accent bar (next to bookmark if present)
...
impl LogViewerApp {
    fn render_find_tab(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_space(8.0);
            let response = ui.add(TextEdit::singleline(&mut self.find_what).desired_width(280.0).font(FontId::monospace(12.0)));
            if response.changed() { self.recompute_find_matches(); }
        });
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                if ui.add(Button::new(RichText::new("Count").color(COL_TEXT).font(FontId::proportional(12.0)))
                    .fill(Color32::from_rgb(40,50,65)).stroke(Stroke::new(0.5,COL_BORDER))
                    .rounding(Rounding::same(4.0)).min_size(Vec2::new(140.0,30.0))).clicked()
                { let n=self.count_matches(); self.status=format!("Found {} matches",n); }

                if ui.add(Button::new(RichText::new("Find All").color(COL_TEXT).font(FontId::proportional(12.0)))
                    .fill(Color32::from_rgb(40,50,65)).stroke(Stroke::new(0.5,COL_BORDER))
                    .rounding(Rounding::same(4.0)).min_size(Vec2::new(140.0,30.0))).clicked()
        ...
        ui.horizontal(|ui| {
            ...
            ui.vertical(|ui| {
                ...
                if ui.add(Button::new(RichText::new("Find Next").color(COL_TEXT).font(FontId::proportional(12.0)))
                    ...
                    .rounding(Rounding::same(4.0)).min_size(Vec2::new(140.0,30.0))).clicked()
                { self.find_next(); }
                ui.add(Button::new(RichText::new("Replace").color(COL_MUTED).font(FontId::proportional(12.0)))
                    .fill(Color32::from_rgb(40,50,65)).stroke(Stroke::new(0.5,COL_BORDER))
                    .rounding(Rounding::same(4.0)).min_size(Vec2::new(140.0,30.0)));
                ui.add(Button::new(RichText::new("Replace All").color(COL_MUTED).font(FontId::proportional(12.0)))
                    .fill(Color32::from_rgb(40,50,65)).stroke(Stroke::new(0.5,COL_BORDER))
                    .rounding(Rounding::same(4.0)).min_size(Vec2::new(140.0,30.0)));
                ui.label(RichText::new("(read-only log)").font(FontId::proportional(10.0)).color(COL_FAINT));
            });
        });
    }
    ...
    fn render_find_in_files_tab(&mut self, ui: &mut egui::Ui) {
        ui.vertical_centered(|ui| {
            ...
            ui.add_space(8.0);
            ui.label(RichText::new("Search across multiple log files.\nSelect a folder to search.")
                .font(FontId::proportional(11.0)).color(COL_MUTED));
            ui.add_space(16.0);
            ui.add(primary_button("Browse for Folder…"));
        });
    }
    ...
    fn render_mark_tab(&mut self, ui: &mut egui::Ui) {
        ui.vertical_centered(|ui| {
            ...
            ui.add_space(8.0);
            ui.label(RichText::new("Bookmark all lines matching the search criteria.")
                .font(FontId::proportional(11.0)).color(COL_MUTED));
            ui.add_space(16.0);
            if ui.add(primary_button("Bookmark All Matches")).clicked() {
                self.recompute_find_matches();
                for &row_idx in &self.find_matches.clone() {
                    if !self.is_bookmarked(row_idx) { self.bookmarks.push(row_idx); }
                }
                self.bookmarks.sort_unstable();
                self.recompute_nav();
            }
